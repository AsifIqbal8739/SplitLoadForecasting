...
class Exp_Split(Exp_Basic):
    def __init__(self, args):
        super(Exp_Split, self).__init__(args)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                if (epoch > 0) and (epoch < (self.args.train_epochs - 1)):
                    compute_MI = False
                elif i%50==0:
                    compute_MI = True
                else:
            ...
            # MI Score Storage
            if self.args.cal_MI and compute_MI and (epoch == 0 or epoch == (self.args.train_epochs - 1)):
                if epoch == 0:
                    epoch_MI_Scores = np.expand_dims(batch_MI_Scores, axis=0)
                else:
                    epoch_MI_Scores = np.concatenate((epoch_MI_Scores,
                                                      np.expand_dims(batch_MI_Scores, axis=0)), axis=0)
        ...
        if self.args.cal_MI:
            ...
            with open(folder_path + 'MI_Scores.npy', 'wb') as f:
