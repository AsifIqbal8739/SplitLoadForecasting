...
class Model(torch.nn.Module):
    def __init__(self, configs):
        ...
        self.MICalc = hmm.MICalculator(d1=self.configs.enc_in + 4, d2=self.configs.d_model)
    ...
    def train(self):
        self.training_MI = True    # to be used in MI calc setup
        self.client.train()
        self.server.train()

    def eval(self):
        self.training_MI = False
        self.client.eval()
        self.server.eval()

    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, compute_MI=False, dp_eps=10,
        ...
        if self.configs.noise_add and self.training_MI: # 2nd check for when performing testing
            b = 2.0
            csie = (csie.detach() + torch.tensor(np.random.laplace(loc=0.0, scale=b, size=csie.shape),
            ...
            # csidx = torch.zeros(size=csidx.shape, dtype=torch.float32, device=device).requires_grad_(True)

        # Apply differential Privacy on outputs of Split 1 -> csie, and csidx
        if self.configs.DP and self.training_MI: # only for training, not for testing
            csie, csidx = self.add_DP(csie, csidx, self.configs.epsDP)

        # Compute Mutual Info between x_enc, x_mark_enc, and csie
        if self.configs.cal_MI and self.training_MI and compute_MI:
            # Mutual Information calculation setup to ensure fresh network
           MI_result = self.MICalc(x_enc, x_mark_enc, csie, pplot=0)
        ...
        #else:
        if self.configs.cal_MI and self.training_MI and compute_MI:
            return dec_out[:, -self.pred_len:, :], MI_result  # [B, L, D]
        else:
            return dec_out[:, -self.pred_len:, :]

    # Perform Differential Privacy by Laplace Mechanism
    def add_DP(self, csie, csidx, dp_eps):
        min_diff = 1e-5
        csie_data = csie.data.cpu().numpy()
        csidx_data = csidx.data.cpu().numpy()
        csie_bound = np.dstack((np.max(csie_data, axis=0), np.min(csie_data, axis=0)))
        csidx_bound = np.dstack((np.max(csidx_data, axis=0), np.min(csidx_data, axis=0)))

        for jj in range(csie_data.shape[1]):
            for kk in range(csie_data.shape[2]):
                csie_sens = min_diff + csie_bound[jj][kk][0] - csie_bound[jj][kk][1]
                csidx_sens = min_diff + csidx_bound[jj][kk][0] - csidx_bound[jj][kk][1]

                csie_dp = Laplace(epsilon=dp_eps, sensitivity=csie_sens)
                csidx_dp = Laplace(epsilon=dp_eps, sensitivity=csidx_sens)
                for ii in range(csie_data.shape[0]):
                    csie_data[ii, jj, kk] = csie_dp.randomise(csie_data[ii, jj, kk])
                    csidx_data[ii, jj, kk] = csidx_dp.randomise(csidx_data[ii, jj, kk])
        csie.data = torch.tensor(csie_data).requires_grad_(True).to(device)
        csidx.data = torch.tensor(csidx_data).requires_grad_(True).to(device)
        return csie, csidx
